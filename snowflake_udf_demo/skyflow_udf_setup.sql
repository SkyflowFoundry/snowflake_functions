...
CREATE OR REPLACE DATABASE SKYFLOW_DEMO_<TODO: SUFFIX>;
USE DATABASE SKYFLOW_DEMO_<TODO: SUFFIX>;

-- Step 2: Create table with PII and Non-PII columns
-- Set context to the database and schema
USE DATABASE SKYFLOW_DEMO_<TODO: SUFFIX>;
USE SCHEMA PUBLIC;
...
INSERT INTO CUSTOMERS (NAME, EMAIL, PHONE, ADDRESS, LIFETIME_PURCHASE_AMOUNT, CUSTOMER_SINCE)
SELECT 
    CONCAT(
...
CREATE OR REPLACE SECRET SKYFLOW_VAULT_SECRET_<TODO: SUFFIX>
    TYPE = GENERIC_STRING
...
CREATE OR REPLACE NETWORK RULE SKYFLOW_APIS_NETWORK_RULE_<TODO: SUFFIX> -- Grant access to the Skyflow API endpoints for authentication and vault APIs
 MODE = EGRESS
 TYPE = HOST_PORT
 VALUE_LIST = ('ebfc9bee4242.vault.skyflowapis.com', 'manage.skyflowapis.com');
 
CREATE OR REPLACE EXTERNAL ACCESS INTEGRATION SKYFLOW_EXTERNAL_ACCESS_INTEGRATION_<TODO: SUFFIX> -- Create an integration using the network rule and secret
 ALLOWED_NETWORK_RULES = (SKYFLOW_APIS_NETWORK_RULE_<TODO: SUFFIX>)
 ALLOWED_AUTHENTICATION_SECRETS = (SKYFLOW_VAULT_SECRET_<TODO: SUFFIX>)
 ENABLED = true;
...
EXTERNAL_ACCESS_INTEGRATIONS = (SKYFLOW_EXTERNAL_ACCESS_INTEGRATION_<TODO: SUFFIX>)
PACKAGES = ('snowflake-snowpark-python', 'pandas', 'pyjwt', 'cryptography', 'requests', 'simplejson', 'cachetools')
SECRETS = ('cred' = SKYFLOW_VAULT_SECRET_<TODO: SUFFIX>)
AS
$$
...
def create_masking_policy(session, policy_name):
    ...
    CREATE OR REPLACE MASKING POLICY {policy_name} AS (VAL STRING) RETURNS STRING ->
      CASE
        WHEN CURRENT_ROLE() IN ('ROLE_AUDIT_ADMIN_<TODO: SUFFIX>') THEN SKYFLOW_DETOKENIZE(VAL, 'PLAIN_TEXT', CURRENT_USER())
        WHEN CURRENT_ROLE() IN ('ROLE_DATA_ENGINEER_<TODO: SUFFIX>') THEN SKYFLOW_DETOKENIZE(VAL, 'MASKED', CURRENT_USER())
        WHEN CURRENT_ROLE() IN ('ROLE_MARKETING_<TODO: SUFFIX>') THEN SKYFLOW_DETOKENIZE(VAL, 'REDACTED', CURRENT_USER())
        ELSE VAL
      END;
    """
...
def tokenize_table(session, vault_id, table_name, primary_key, pii_fields_delimited, vault_owner_email):  
    ...
    create_masking_policy(session, 'DETOKENIZE_COLUMN_<TODO: SUFFIX>')
    for field in pii_fields:
        set_masking_policy(session, table_name, field, 'DETOKENIZE_COLUMN_<TODO: SUFFIX>')
...
def create_snowflake_stream(session, vault_id, table_name, primary_key, pii_fields_delimited):
    ...
    # Create the stream
    session.sql(f"""
        CREATE OR REPLACE STREAM SKYFLOW_PII_STREAM_{table_name}_<TODO: SUFFIX>
        ON TABLE {table_name}
    """).collect()

    # Create the task using the current warehouse
    session.sql(f"""
        CREATE OR REPLACE TASK SKYFLOW_PII_STREAM_{table_name}_<TODO: SUFFIX>_TASK
        WAREHOUSE = '{warehouse_name}'
        SCHEDULE = '1 MINUTE'
        WHEN SYSTEM$STREAM_HAS_DATA('SKYFLOW_PII_STREAM_{table_name}_<TODO: SUFFIX>')
    ...
    # Resume the task
    session.sql(f"""
        ALTER TASK SKYFLOW_PII_STREAM_{table_name}_<TODO: SUFFIX>_TASK RESUME
    """).collect()
...
def SKYFLOW_PROCESS_PII(session, vault_id, table_name, primary_key, pii_fields):
    ...
    stream_records = session.sql(f"SELECT {primary_key}, METADATA$ACTION, {', '.join(pii_fields_list)} FROM SKYFLOW_PII_STREAM_{table_name}_<TODO: SUFFIX>").collect()
    ...
    session.sql(f'CREATE OR REPLACE STREAM SKYFLOW_PII_STREAM_{table_name}_<TODO: SUFFIX> ON TABLE SKYFLOW_DEMO_<TODO: SUFFIX>.PUBLIC.{table_name}').collect()
    return "Changes processed"

$$;

-- Create roles
CREATE ROLE ROLE_AUDIT_ADMIN_<TODO: SUFFIX>;
CREATE ROLE ROLE_DATA_ENGINEER_<TODO: SUFFIX>;
CREATE ROLE ROLE_MARKETING_<TODO: SUFFIX>;

-- Grant usage on database SKYFLOW_DEMO to roles
GRANT USAGE ON DATABASE SKYFLOW_DEMO_<TODO: SUFFIX> TO ROLE ROLE_AUDIT_ADMIN_<TODO: SUFFIX>;
GRANT USAGE ON DATABASE SKYFLOW_DEMO_<TODO: SUFFIX> TO ROLE ROLE_DATA_ENGINEER_<TODO: SUFFIX>;
GRANT USAGE ON DATABASE SKYFLOW_DEMO_<TODO: SUFFIX> TO ROLE ROLE_MARKETING_<TODO: SUFFIX>;

-- Grant usage on schema SKYFLOW_DEMO.PUBLIC to roles
GRANT USAGE ON SCHEMA SKYFLOW_DEMO_<TODO: SUFFIX>.PUBLIC TO ROLE ROLE_AUDIT_ADMIN_<TODO: SUFFIX>;
GRANT USAGE ON SCHEMA SKYFLOW_DEMO_<TODO: SUFFIX>.PUBLIC TO ROLE ROLE_DATA_ENGINEER_<TODO: SUFFIX>;
GRANT USAGE ON SCHEMA SKYFLOW_DEMO_<TODO: SUFFIX>.PUBLIC TO ROLE ROLE_MARKETING_<TODO: SUFFIX>;

-- Grant access to table SKYFLOW_DEMO.PUBLIC.CUSTOMERS to roles
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE SKYFLOW_DEMO_<TODO: SUFFIX>.PUBLIC.CUSTOMERS TO ROLE ROLE_AUDIT_ADMIN_<TODO: SUFFIX>;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE SKYFLOW_DEMO_<TODO: SUFFIX>.PUBLIC.CUSTOMERS TO ROLE ROLE_DATA_ENGINEER_<TODO: SUFFIX>;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE SKYFLOW_DEMO_<TODO: SUFFIX>.PUBLIC.CUSTOMERS TO ROLE ROLE_MARKETING_<TODO: SUFFIX>;
...
CALL GRANT_WAREHOUSE_ACCESS('ROLE_AUDIT_ADMIN_<TODO: SUFFIX>');
CALL GRANT_WAREHOUSE_ACCESS('ROLE_DATA_ENGINEER_<TODO: SUFFIX>');
CALL GRANT_WAREHOUSE_ACCESS('ROLE_MARKETING_<TODO: SUFFIX>');
...
CALL GRANT_ROLE_TO_CURRENT_USER('ROLE_AUDIT_ADMIN_<TODO: SUFFIX>');
CALL GRANT_ROLE_TO_CURRENT_USER('ROLE_DATA_ENGINEER_<TODO: SUFFIX>');
CALL GRANT_ROLE_TO_CURRENT_USER('ROLE_MARKETING_<TODO: SUFFIX>');
