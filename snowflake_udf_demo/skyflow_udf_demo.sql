...
USE ROLE ROLE_DATA_ENGINEER_<TODO: SUFFIX>;
SELECT * FROM CUSTOMERS;
...
USE ROLE ROLE_MARKETING_<TODO: SUFFIX>;
SELECT * FROM CUSTOMERS;

-- Query to calculate the average lifetime purchase amount across all customers
SELECT AVG(LIFETIME_PURCHASE_AMOUNT) AS AVG_PURCHASE_AMOUNT FROM CUSTOMERS;
...
SELECT CUSTOMER_ID, LIFETIME_PURCHASE_AMOUNT FROM CUSTOMERS ORDER BY LIFETIME_PURCHASE_AMOUNT DESC LIMIT 10;

-- Query to get the top 10 customers by lifetime purchase amount... as well as their phone number and email address...
SELECT CUSTOMER_ID, PHONE, EMAIL, LIFETIME_PURCHASE_AMOUNT FROM CUSTOMERS ORDER BY LIFETIME_PURCHASE_AMOUNT DESC LIMIT 10;

-- Step 6: Switch to ROLE_AUDIT_ADMIN role (Full PII access) and query the data again
USE ROLE ROLE_AUDIT_ADMIN_<TODO: SUFFIX>;
...
SELECT * FROM SKYFLOW_PII_STREAM_CUSTOMERS_<TODO: SUFFIX> SHOW_CURSOR;
SELECT STATE, ERROR_MESSAGE, SCHEDULED_TIME
FROM TABLE(INFORMATION_SCHEMA.TASK_HISTORY(TASK_NAME=>'SKYFLOW_PII_STREAM_CUSTOMERS_<TODO: SUFFIX>_TASK'))
