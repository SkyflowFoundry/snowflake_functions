...
CREATE OR REPLACE FUNCTION code_schema.skyflowDetokenize(vault_url string, token string)
  RETURNS string
  LANGUAGE python
  runtime_version = '3.8'
  packages = ('snowflake-snowpark-python', 'pyjwt', 'cryptography', 'requests', 'simplejson')
...
GRANT USAGE ON FUNCTION code_schema.skyflowDetokenize(string, string) TO APPLICATION ROLE app_public;